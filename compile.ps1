...
$code | Set-Content -Path "src/sqlbuild.cs"
...
$code = (Get-Content -path "src/msbuild.proj" -Raw)
...
$code | Set-Content -Path "dist/msbuild.proj"

# Compile the powershell script.
$code = (Get-Content -path "src/powershell.ps1" -Raw)
$code = $code -replace ([regex]::Escape('{sql.install.sql}')), $sqlInstall
$code | Set-Content -Path "dist/powershell.ps1"